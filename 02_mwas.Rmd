...
res_df_unique <- res_df |> 
  select(sub_pathway, super_pathway)|>
  distinct(sub_pathway, super_pathway, .keep_all = TRUE) |>
  rename(pathway = sub_pathway)
```
...
results_df <- results |>
  left_join(res_df_unique, by = "pathway")

write_csv(results_df, "~/Documents/air_polln_metabolomics/result/lm_mwas/mwas_pathway_ora_results.csv")
```

#ora results
```{r}
ora_pathway_res<- read_csv("~/Documents/air_polln_metabolomics/result/lm_mwas/mwas_pathway_ora_results.csv") |>
  filter(fdr<0.2) |>
  mutate(
    exposure = case_when(
      str_starts(variable, "bc") ~ "Black carbon",
  ...
  mutate(enrichment_ratio = if_else(enrichment_ratio>15, 15, enrichment_ratio),
         exposure_window = str_remove(exposure_window, " month$")
         ) |>
  mutate(super_pathway = recode(super_pathway,
                               'Cofactors and Vitamins' = 'Vit&Ene',
                               'Energy' = 'Vit&Ene'))
```

#visualizing results - Summarized heat map
```{r}
ggplot(ora_pathway_res, aes(x = exposure_window, y = pathway, fill = enrichment_ratio)) +
  ...
  labs(x = "Gestational months", y = "") +
  theme_minimal() +
  theme(axis.text.x = element_text(size=12, angle = 0, hjust = 0.5, vjust = 0.5, color="black"),
        ...
        strip.text.y = element_text(size=8, angle = 45, hjust = 0, color="black"),
...
# Calculate total unique chemicals per sub_pathway before filtering
total_chemicals <- res_df |>
  filter(!str_detect(variable, "cnt$")) |>
  mutate(chemical_name = str_to_title(chemical_name)) |>
  distinct(sub_pathway, chemical_name) |>
  count(sub_pathway, name = "total_chemicals")
...
summary_df2 <- res_df |>
  ...
  mutate(month_count = lengths(months)) |>
  (\(data) {
  ...
  group_by(sub_pathway, prefix, sex) |>
  mutate(n_chemicals = n_distinct(chemical_name)) |>
  ungroup() |>
  left_join(total_chemicals, by = "sub_pathway") |>
  mutate(chemical_fraction = paste0(n_chemicals, "/", total_chemicals))
...
res_df <- read_csv("~/Documents/air_polln_metabolomics/result/lm_mwas/placenta_mwas_oct_2025_anno.csv") |>
  ...
  clean_names() |>
  filter(fdr < 0.2, 
         sub_pathway %in% ora_pathway_res$pathway)

# Step 1: Calculate TOTAL chemical counts per sub_pathway (from full dataset)
chem_counts <- read_csv("~/Documents/air_polln_metabolomics/result/lm_mwas/placenta_mwas_oct_2025_anno.csv") |>
    select(chemical_name, sub_pathway) |> 
    distinct() |>
    mutate(sub_pathway = str_remove(sub_pathway, " Metabolism$")) |>
    group_by(sub_pathway) |>
    summarise(
        n_chemicals_total = n_distinct(chemical_name),
        .groups = "drop"
    )

# Step 1b: Calculate TOTAL sub_pathway counts per super_pathway (from full dataset)
subpathway_counts <- read_csv("~/Documents/air_polln_metabolomics/result/lm_mwas/placenta_mwas_oct_2025_anno.csv") |>
    ...
    group_by(super_pathway) |>
    summarise(
        n_subpathways_total = n_distinct(sub_pathway),
        .groups = "drop"
    )

# Step 2: Create summary table with counts PER EXPOSURE
summary_df <- res_df |>
    mutate(
        exposure = case_when(
            str_starts(variable, "bc") ~ "Black carbon",
    ...
    summarise(
        n_chem_sig = n_distinct(chemical_name),
        .groups = "drop"
    ) |>
    left_join(chem_counts, by = "sub_pathway") |>
    mutate(
        percent_chem = round((n_chem_sig / n_chemicals_total) * 100, 2)
    )

# Step 3: Count unique sub_pathways per super_pathway PER EXPOSURE
sig_subpathways <- summary_df |>
    group_by(exposure, super_pathway) |>
    summarise(
        n_subpathways_sig = n_distinct(sub_pathway),
        .groups = "drop"
    )

# Step 4: Join and calculate percentages
summary_df <- summary_df |>
    left_join(sig_subpathways, by = c("exposure", "super_pathway")) |>
    left_join(subpathway_counts, by = "super_pathway") |>
    mutate(
        percent_sub_pathway = round((n_subpathways_sig / n_subpathways_total) * 100, 2)
    )

# Step 5: Get the percentages for the OTHER exposure to include in labels
summary_other_exposure <- summary_df |>
    select(exposure, super_pathway, sub_pathway, percent_chem, percent_sub_pathway) |>
    mutate(
        other_exposure = ifelse(exposure == "Black carbon", "PM2.5", "Black carbon")
    ) |>
    select(other_exposure, super_pathway, sub_pathway, 
           percent_chem_other = percent_chem, 
           percent_sub_pathway_other = percent_sub_pathway)

# Step 6: Join with other exposure data and format labels
summary_df_final <- summary_df |>
    select(exposure, super_pathway, sub_pathway, n_chem_sig, 
           percent_chem, percent_sub_pathway) |>
    left_join(
        summary_other_exposure,
        by = c("exposure" = "other_exposure", "super_pathway", "sub_pathway")
    ) |>
    mutate(
        # Format labels based on current exposure
        sub_pathway = case_when(
            exposure == "Black carbon" ~ paste0(
                sub_pathway, " (",
                round(percent_chem), "|",
                round(coalesce(percent_chem_other, 0)), ")"
            ),
            exposure == "PM2.5" ~ paste0(
                sub_pathway, " (",
                round(percent_chem_other), "|",   # BC comes first
                round(percent_chem), ")"
            )
        ),
        super_pathway = case_when(
            exposure == "Black carbon" ~ paste0(
                super_pathway, " (",
                round(percent_sub_pathway), "|",
                round(coalesce(percent_sub_pathway_other, 0)), ")"
            ),
            exposure == "PM2.5" ~ paste0(
                super_pathway, " (",
                round(percent_sub_pathway_other), "|",   # BC comes first
                round(percent_sub_pathway), ")"
            )
        )
    ) |>
    select(exposure, super_pathway, sub_pathway, n_chem_sig, percent_chem, percent_sub_pathway) |>
  rename(n_chem = n_chem_sig)

write_csv(summary_df_final, "~/Documents/air_polln_metabolomics/result/lm_mwas/sankey_plt.csv")
```
...
#summary for article text [Table S3]
```{r}
...
res_df <- read_csv("~/Documents/air_polln_metabolomics/result/lm_mwas/placenta_mwas_oct_2025_anno.csv") |>
  ...
  clean_names() |>
  filter(fdr < 0.2) |>
  semi_join(ora_pathway_res, by = c("sub_pathway" = "pathway", 
                                    "variable" = "variable",
                                    "sex" = "sex"))
...
```{r}
# get ora_pathway_res dataframe from line 351

text_bc<- ora_pathway_res |>
  filter(
    fdr<0.2,
    sex == "overall",
         exposure == "Black carbon",
         super_pathway %in% c("Peptide"),
         pathway == "Gamma-glutamyl Amino Acid") 

text_pm<- ora_pathway_res |>
  filter(sex == "overall",
         exposure == "PM2.5",
         #super_pathway %in% c("Nucleotide"),
         #pathway == "Purine Metabolism, Adenine containing"
         ) 


#find most overlapping metabolites
text_pm$significant_metabolites %>%
  # remove parenthetical content, split on ';' and trim
  str_remove_all("\\(.*?\\)") %>%
  str_split(";") %>%
  map(~ str_trim(.x)) %>%
  # remove empty entries and trailing asterisks, make unique per observation
  map(~ .x[.x != ""] %>% str_replace_all("\\*+$", "") %>% unique()) %>%
  # flatten to one vector of "observation-presence" and tabulate
  imap_dfr(~ tibble(metabolite = .x, obs = .y)) %>%  # .y = observation index
  distinct(metabolite, obs) %>%                      # ensure one count per obs
  count(metabolite, name = "n_obs") %>%              # how many observations contain it
  arrange(desc(n_obs))

#repeated pathways over time
# Step 1: summarize repeated pathways across windows
pathway_repeat_summary <- ora_pathway_res %>%
  group_by(exposure, sex, pathway) %>%
  summarize(
    window_count = n_distinct(exposure_window),
    windows = paste(sort(unique(exposure_window)), collapse = ", "),
    .groups = "drop"
  )

# Step 2: metabolite repetition across windows
metabolite_rep_summary <- ora_pathway_res %>%
  group_by(exposure, sex, pathway, exposure_window) %>% 
  summarise(
    metabolites = list(
      significant_metabolites %>%
        ...
        str_replace_all("\\*+$", "") %>%  # remove trailing *
        .[. != ""] %>%                    # remove empties
        unique()
    ),
    .groups = "drop"
  ...
  unnest(metabolites) %>% 
  distinct(exposure, sex, pathway, exposure_window, metabolites) %>%
  count(exposure, sex, pathway, metabolites, name = "n_windows") %>%
  group_by(exposure, sex, pathway) %>%
  arrange(desc(n_windows), metabolites) %>%
  slice_head(n = 3) %>%   # <-- TOP 3 METABOLITES
  mutate(metabolite_fmt = paste0(metabolites, " (", n_windows, ")")) %>%
  summarize(
    top_metabolites = paste(metabolite_fmt, collapse = ", "),
    .groups = "drop"
  )

# Step 3: merge with pathway summary
final_result <- pathway_repeat_summary %>%
  left_join(metabolite_rep_summary, by = c("exposure", "sex", "pathway"))

#female paragraph
res_fem<- final_result |>
  filter(sex == "Female")
