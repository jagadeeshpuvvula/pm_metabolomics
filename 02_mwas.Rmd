...
# Set filter options
filter_type <- "any"  # Change to "any" for any N windows
n_windows <- 1  # Change to desired number of windows
...
summary_df2 <- res_df |>
  ...
  ungroup() |>
  left_join(total_chemicals, by = "sub_pathway") |>
  filter(total_chemicals>3) |>
  mutate(chemical_fraction = paste0(n_chemicals, "/", total_chemicals),
         chem_frac_n = n_chemicals/total_chemicals) |>
  select(-c(8:10))


x<- summary_df2 |>
  filter(
    sub_pathway == "Pyrimidine Metabolism, Cytidine containing",
    #super_pathway == "Peptide",
    prefix == "pm",
    sex == "Female"
  )
```
