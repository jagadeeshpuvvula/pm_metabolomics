...
ggplot(sctr_viz, aes(x = anomaly_days, y = bw_z_scr, color = sex)) +
  geom_point(alpha = 0.3) +
  geom_smooth(method = "lm",  se=F) +
  facet_grid(exposure ~ exposure_window, scale= "free") +
  theme_minimal() +
  scale_color_manual(values = c("Male" = "darkblue", "Female" = "darkred")) +
  labs(
    x = "Count of days with high air pollutant exposures",     
    y = "Birth weight Z-Score"        
  )+
  theme(legend.position = "bottom")

ggsave("~/Documents/air_polln_metabolomics/result/exposure_bw_zscore_scatter.tiff",
       dpi=300,
       bg="white",
       width = 10,
       height = 5)
...
viz_df<- res_df |>
  ...
  super_pathway = fct_relevel(super_pathway, 
                               ...
                               "Other"),
  sex = case_match(sex,
                   "overall" ~ "Overall", "Male" ~ "Male", "Female" ~ "Female", .default = sex),
  sex = factor(sex, levels = c("Overall", "Male", "Female"))
  ) 
```
