...
M_matrix <- apply(analysis_df[, 54:960], 2, as.numeric)
X_numeric <- as.numeric(analysis_df$pm_9month)
Y_numeric <- as.numeric(analysis_df$bw_z_scr)

# Select the covariate columns
covariate_cols <- c("race_ethnicity", "education", "tobacco_use_dur_preg", 
                    "parity", "bmi", "maternal_age")

covariate_cols <- c("tobacco_use_dur_preg", "bmi")

# Create covariate matrix with dummy variables for factors
COV_XM <- model.matrix(~ . - 1, data = analysis_df[, covariate_cols])

# Optional: Check data structure (can comment out after verification)
print(paste("Sample size:", length(X_numeric)))
print(paste("Number of mediators:", ncol(M_matrix)))
print(paste("X range:", min(X_numeric), "to", max(X_numeric)))
print(paste("Y range:", min(Y_numeric), "to", max(Y_numeric)))

# Run HIMA classic mediation analysis
...
```{r}
summary(lm(bw_z_scr ~ pm_2month + tobacco_use_dur_preg + bmi, data = analysis_df))
```
